...
// 身份检查：先加载云端旅行和真实 members，再验证本地绑定。
onMounted(async () => {
  if (!trip.value) await store.loadTripById(tripId)
...
async function doRemoveMember(memberId: string) {
  const removingMyself = store.getMyMemberId(tripId) === memberId
  if (removingMyself) {
    const confirmed = window.confirm('你正在删除自己的身份，删除后需要重新选择身份。')
    if (!confirmed) return
  }
  ...
  if (ok) {
    editingMember.value = null
    if (removingMyself && trip.value?.shareCode) {
      router.replace(`/join/${trip.value.shareCode}`)
    }
  } else {
